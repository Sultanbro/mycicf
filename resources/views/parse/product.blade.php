        ...
        <div class="col-12">
            ...
            <div class="majorMargin flex_colclear contentmargin">
                <div class="flex_colclear">	<!--Кнопка переключения таблиц-->
                    <div class="flexrowClear">
                        <a onclick="getCompaniesByProduct(0)"><div class="topButton pointer">
                                топ по компаниям
                            </div></a>
                    ...
                    <div>
                        <div>
                            <div class="table-responsive">
                                <div>
                                    <table class="table table-bordered table-hover t_table">
                                        <thead>
                                        <tr><th colspan="7" class="textRight tableTopMarkup">премии</th><th colspan="6" class="textRight tableTopMarkup">выплаты</th></tr>
                                        <tr>
                                            <th>Класс страхования</th>
                                            <th>{{$label_first}}</th>
                                            <th>{{$label_second}}</th>
                                            <th>доля {{$label_first}}</th>
                                            <th>доля {{$label_second}}</th>
                                            <th>изм %</th>
                                            <th>изм сумма</th>
                                        ...
                                        @foreach($insuranceClassList as $id => $key)
                                            <tr  class="fontbold">
                                                <td><span><a onclick="getCompaniesByClass({{$id}})">{{$controller->getNameByClassId($id)}}</a></span></td>
                                                <td><span>{{number_format($class_sum[$id]['premium_first'], 0, '.', ' ')}}</span></td>
                                                <td><span>{{number_format($class_sum[$id]['premium_second'], 0, '.', ' ')}}</span></td>
                                                <td><span>{{$controller->getPercentOfMarker($class_sum[$id]['premium_first'], array_sum($premium_first))}}</span></td>
                                                <td><span>{{$controller->getPercentOfMarker($class_sum[$id]['premium_second'], array_sum($premium_second))}}</span></td>
                                                <td><span>{{$controller->getChangedVal($class_sum[$id]['premium_first'], $class_sum[$id]['premium_second'])}}</span></td>
                                                <td><span>{{number_format($class_sum[$id]['premium_second'] - $class_sum[$id]['premium_first'], 0, '.', ' ') }}</span></td>
                                                <td><span></span></td>
                                                <td><span>{{number_format($class_sum[$id]['payout_first'], 0, '.', ' ')}}</span></td>
                                                <td><span>{{number_format($class_sum[$id]['payout_second'], 0, '.', ' ')}}</span></td>
                                                <td><span>{{$controller->getChangedVal($class_sum[$id]['payout_first'],$class_sum[$id]['payout_second'])}}</span></td>
