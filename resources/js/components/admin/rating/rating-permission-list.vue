    ...
    <div>
        ...
        <table class="table table-responsive-sm table-stripper table-data table-bordered">
            ...
            <tr v-for="item in list">
                <td>{{item.id}}</td>
                <td>{{item.name}}</td>
                <td> <span v-if="item.role == 'rating'">Рейтинг продваца</span></td>
                <td>{{item.date}}</td>
                <td>{{item.changer}}</td>
                <td>
...
<script>
    export default {
        ...
        methods: {
            getList() {
                this.axios.post('/rating/getKuratorsList', {})
                    .then( response => {
            ...
            deleteItem(id){
                this.axios.get(`/rating/delete/${id}`);
                alert('Успешно удалено');
            },
            addUser(){
                location.href = "/rating/newUser";
            }
        },
