...
export function LikedUsers({ postId }: LikedUsersProps) {
  ...
  return (
    <Card
      style={{ boxShadow: "0 0 8px #0000003b" }}
...
export function PostLike({ post }: PostLikeProps) {
  ...
  let icon = isLiked ? (
    <LikeTwoTone style={{ color: "red" }} />
  ) : (
    <LikeOutlined />
  );

  let PostLikeAjaxButton = ({
    ...props
  }: AjaxButtonProps<LikeRequest, LikeResponse>) => (
  ...
  return (
    ...
    >
      {(() => {
        let span = <span>{likes} Нравится</span>;
        ...
        return (
          <Dropdown
            overlay={<LikedUsers postId={post.postId} />}
            placement="topCenter"
          >
            {span}
          </Dropdown>
