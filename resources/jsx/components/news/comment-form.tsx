...
let commentForm = (
  { postId, onCommendAdded, text = "" }: CommentFormProps,
  ref: ForwardedRef<any>
) => {
  let useLocalStorage = createUseLocalStorage(`post:${postId}:`);
  let [commentText, setCommentText] = useLocalStorage("commentText", text);
  let AddCommentAjaxButton = ({
    ...props
  }: AjaxButtonProps<CommentAjaxRequest, CommentAjaxResponse>) => (
  ...
  return (
    <div ref={ref}>
      <Row>
        ...
        <Col md={2}>
          <AddCommentAjaxButton
            url="/news/comments/addComment"
            method="POST"
            type="text"
            disabled={!commentText}
            icon={<SendOutlined />}
            data={{ isn: authUserIsn(), commentText, postId }}
            onSuccess={(res) => {
              setCommentText("");
...
export let CommentForm = React.forwardRef<HTMLDivElement, CommentFormProps>(
  commentForm
) as any;
